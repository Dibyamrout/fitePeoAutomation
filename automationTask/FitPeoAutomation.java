...
public class FitPeoAutomation {
	...
	private static void navigateToRevenueCalculator(WebDriver driver, WebDriverWait wait) {
		WebElement revenueCalculatorLink = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[@href='/revenue-calculator']")));
		revenueCalculatorLink.click();
		System.out.println("Navigated to Revenue Calculator page.");
	}

	private static void adjustSlider(WebDriver driver, WebDriverWait wait) {
		WebElement sliderLabel = wait.until(
	...
	private static void updateTextField(WebDriver driver) throws InterruptedException {
		...
		Thread.sleep(100);
		System.out.println("Updated text field to "+textField.getAttribute("value")+".");
	}
	...
	private static void validateSliderValue(WebDriver driver, String expectedValue) {
		...
		if (!actualValue.equals(expectedValue)) {
			throw new AssertionError(
					"Slider value is incorrect! Expected: " + expectedValue + ", Actual: " + actualValue);
		}
	...
	private static void selectCPTCodes(WebDriver driver, WebDriverWait wait) {
		...
		for (String cpt : cptLabels) {
			try {
				WebElement cptElement = wait
						.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//span[text()='" + cpt + "']")));
